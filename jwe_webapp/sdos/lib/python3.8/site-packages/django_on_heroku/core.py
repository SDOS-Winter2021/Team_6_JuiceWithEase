...
def settings(
    ...
    *,
            ...
            for (env, url) in os.environ.items():
                if env.startswith("HEROKU_POSTGRESQL"):
                    ...
                    config["DATABASES"][db_color] = dj_database_url.parse(
                        url, conn_max_age=conn_max_age, ssl_require=db_ssl_required
                    )
        ...
        if "DATABASE_URL" in os.environ:
            ...
            if geodjango:
                if "postgres" in db_config["ENGINE"]:
                    ...
                    logger.info(
                        "Geodjango is enabled but your $DATABASE_URL don't have a valid GIS engine available."
                    )
    ...
    if staticfiles:
        ...
        middleware_key = (
            _MIDDLEWARE_CLASSES if _MIDDLEWARE_CLASSES in config else _MIDDLEWARE
        )
