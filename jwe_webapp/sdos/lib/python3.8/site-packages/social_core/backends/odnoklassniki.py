...
class OdnoklassnikiApp(BaseAuth):
    ...
    def get_auth_sig(self):
        ...
        hash_source = "{0:s}{1:s}{2:s}".format(
    ...
    def get_response(self):
        ...
        return dict((name, self.data[name]) for name in fields if name in self.data)
...
def odnoklassniki_oauth_sig(data, client_secret):
    ...
    """
    suffix = md5(
        "{0:s}{1:s}".format(data["access_token"], client_secret).encode("utf-8")
    ).hexdigest()
    check_list = sorted(
        [
...
def odnoklassniki_iframe_sig(data, client_secret_or_session_secret):
    ...
    """
    param_list = sorted(
        ["{0:s}={1:s}".format(key, value) for key, value in data.items()]
    )
    return md5(
