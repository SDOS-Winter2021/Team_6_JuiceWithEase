...
class OpenIdConnectAuth(BaseOAuth2):
    ...
    def authorization_url(self):
        return self.AUTHORIZATION_URL or self.oidc_config().get(
            "authorization_endpoint"
        )

    def access_token_url(self):
        return self.ACCESS_TOKEN_URL or self.oidc_config().get("token_endpoint")

    def revoke_token_url(self, token, uid):
        return self.REVOKE_TOKEN_URL or self.oidc_config().get("revocation_endpoint")

    def id_token_issuer(self):
        return self.ID_TOKEN_ISSUER or self.oidc_config().get("issuer")

    def userinfo_url(self):
        return self.USERINFO_URL or self.oidc_config().get("userinfo_endpoint")

    def jwks_uri(self):
        return self.JWKS_URI or self.oidc_config().get("jwks_uri")
